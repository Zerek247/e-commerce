...
import { Instagram, Twitter, Facebook, Youtube, Truck, RefreshCw, Lock, Headphones } from 'lucide-react';

export default function Footer() {
  return (
    <footer className="mt-16 sm:mt-20 md:mt-24">
      {/* TRUST BADGES */}
      <div className="bg-pink-50/60 border-y border-pink-100">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-6 py-8 sm:py-10 grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {[
            { icon: Truck, title: 'Free shipping', text: 'On all orders over $75' },
            { icon: RefreshCw, title: 'Easy returns', text: 'Within 30 days, no questions' },
            { icon: Lock, title: 'Secure payment', text: 'Encrypted & PCI compliant' },
            { icon: Headphones, title: '24/7 support', text: 'Reach our team anytime' },
          ].map(({ icon: Icon, title, text }) => (
            <div key={title} className="flex items-center gap-3 sm:gap-4">
              <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-white border border-pink-200 flex items-center justify-center shrink-0 shadow-soft">
                <Icon size={18} strokeWidth={1.75} className="text-pink-500" />
              </div>
              <div>
                <p className="font-display text-sm sm:text-base text-ink leading-tight">{title}</p>
                <p className="text-[11px] sm:text-xs text-ink-light mt-0.5">{text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* MAIN FOOTER */}
      <div className="bg-gradient-to-br from-ink via-ink-soft to-ink text-white relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-6 py-16 sm:py-20 relative">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-10 mb-12 sm:mb-16">
            ...
            <div className="md:col-span-4">
              ...
              {/* Payment methods */}
              <div className="mt-7">
                <p className="text-[10px] uppercase tracking-[0.25em] text-white/50 mb-3 font-semibold">We accept</p>
                <div className="flex flex-wrap gap-2">
                  {['Visa', 'Master', 'Amex', 'PayPal', 'Apple Pay', 'GPay'].map((m) => (
                    <span
                      key={m}
                      className="text-[10px] tracking-wider font-semibold bg-white/10 border border-white/10 text-white/80 px-3 py-1.5 rounded-md"
                    >
                      {m}
                    </span>
                  ))}
